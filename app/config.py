"""Single source of truth for MCP plugin configuration.
...
Two credentials, three tiers, one resolver:
  ...
  * ``MCP_LLM_API_KEY`` (env default + per-session UI override) authenticates
    DSPy / signatures / embeddings to the LLM provider.
...
# Hard-coded defaults for fields the yaml does not provide. These mirror the
# inputs rendered in the UI's Global Model Config so a fresh checkout has
# sensible behavior even with a minimal yaml.
_NUMERIC_FALLBACKS = {
    "temperature": 0.5,
    "max_tokens": 10000,
    "max_tool_calls": 5,
...
def llm_defaults() -> dict:
    ...
    The returned dict has the same fields as the legacy ``get_llm_config``
    plus an explicit ``fields_locked`` map and an api_key resolved from the
    env var named in ``api_key_env`` (rather than yaml's literal value).
...
def caldera_connection() -> dict:
    ...
    block. Falls back to the long-standing local-dev values when the env
    vars are unset so a fresh checkout works without configuration.
    """
